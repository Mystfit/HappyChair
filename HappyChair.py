        ...
        while True:
            ...
            for i in range(len(scores)):
                ...
                if ((scores[i] > min_conf_threshold) and (scores[i] <= 1.0) and object_name == "person"):
                    ...
                    x_center = int(max(1,(boxes[i][1] * imW))) + int(min(imW,(boxes[i][3] * imW))) * 0.5
                    new_dist_center = abs(screen_center[0] - x_center)
                    prev_dist_center =  abs(screen_center[0] - person_center_x)
                    ...
                    cv2.putText(frame, f"Dist: {new_dist_center}", (int(x_center), int(screen_center[1])-7), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 0), 2)

            
            if found_person:
                cv2.rectangle(frame, (int(person_center_x)-20,int(imH*0.5)-20), (int(person_center_x)+20,int(imH*0.5)+20), (0, 255, 10), 2)
...
videostream = VideoStream(resolution=(imW,imH),framerate=2, flip=flip_webcam).start()
